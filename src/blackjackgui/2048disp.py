...
class Display(Frame):
    def __init__(self):
        ...
        self.commands = {UP_KEY: _2048func.move_up, 
                         DOWN_KEY: _2048func.move_down,
                         LEFT_KEY: _2048func.move_left, 
                         RIGHT_KEY: _2048func.move_right,
                         AI_KEY: _2048ai.ai_move,
                         }
    ...
    def init_matrix(self):
        self.matrix = _2048func.initialize_game()
    ...
    def key_press(self, event):
        ...
        if key == AI_PLAY_KEY:
            move_count = 0
            while valid_game:
                self.matrix, valid_game = _2048ai.ai_move(self.matrix,40, 30)
                if valid_game:
                    self.matrix = _2048func.add_new_tile(self.matrix)
                    self.draw_grid_cells()
                move_count += 1
        if key == AI_KEY:
            self.matrix, move_made = _2048ai.ai_move(self.matrix, 20, 30)
            if move_made:
                self.matrix = _2048func.add_new_tile(self.matrix)
                self.draw_grid_cells()
                move_made = False
