...
class ConfigFrame():
    def __init__(self, parent):
        ...
        playerBtn = Button(buttonFrm, text="Config Player", command=self.popupSetSeat)
        playerBtn.grid(row=0, column=0)
        bgBtn = Button(buttonFrm, text="Config Background", command=self.popupCofigBackground)
    ...
    def popupSetSeat(self):
        ...
        top.geometry(f'255x150+{rootX}+{rootY}') # 300,250 is width,height; 200,200 is topleft
        top.title("Set Player's seat")
        top.configure(bg='yellow')
        Label(top, text="player name:",bg='yellow').grid(row=0,column=0)
        ...
        Label(top, text="Available Seats:",bg='yellow').grid(row=1, column=0)
    ...
    def popupCofigBackground(self):
        ...
        for text, num in precolors:
            b = Radiobutton(top, text=text, variable=self.colorVar, value=num)
            b.pack()
        setBtn = Button(top, text="Set", width=20, command=self.setBackground)
        setBtn.place(x=55, y=120)

    def setBackground(self):
        print(self.colorVar.get())
