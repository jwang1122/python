...
class PlayBoard(BaseWindow):
    ...
    def displayPlayerNames(self):
        for player in self.playerList:
            ...
            cardImage = ImageTk.PhotoImage(file='src/images/diamondK.gif')
            imageLbl = Label(self.root)
            imageLbl.configure(image=cardImage)
            imageLbl.image = cardImage
            imageLbl.place(x=player.getCardX(), y=player.getCardY())
